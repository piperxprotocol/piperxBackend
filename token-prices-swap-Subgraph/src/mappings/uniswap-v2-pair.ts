...
export function handleV2Swap(event: Swap): void {
  ...
  if (!pair) {
    log.warning("TokenPair not found for V2 swap: {}", [pairId]);
    return;
  }
  ...
  const token0Amount = event.params.amount0In.plus(event.params.amount0Out);
  const token1Amount = event.params.amount1In.plus(event.params.amount1Out);
  ...
  log.info("✅ V2 Swap handled for pair: {}", [pairId]);
}
